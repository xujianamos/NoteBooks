...
> **说明**：
>
> 1. `str`类型转`int`类型时可以通过`base`参数来指定进制，可以将字符串视为对应进制的整数进行转换。
>
> 2. `str`类型转成`bool`类型时，只要字符串有内容，不是`''`或`""`，对应的布尔值都是`True`。
> 3. `bool`类型转`int`类型时，`True`会变成`1`，`False`会变成`0`。
> 4. 在 ASCII 字符集和 Unicode 字符集中， 字符`'d'`对应的编码都是`100`。
